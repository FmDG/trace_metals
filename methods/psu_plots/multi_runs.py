...
def psu_plots_by_run(min_age=2400, max_age=2900, num_runs=2, save_fig=False, figure_name="fig", fill=True, core=1209):
    ...
    for x in range(num_runs):
        ...
        axs[0].plot(datasets[x][0]["age_ka"], datasets[x][0]["temp"], color=colours[x], linestyle='-', label="{} Run {}".format(datasets[x][1], (x + 1)))
        # and if desired, fill in the error bars
        if fill:
            axs[0].fill_between(datasets[x][0]["age_ka"], datasets[x][0]["temp_min1"], datasets[x][0]["temp_plus1"], alpha=0.1, facecolor=colours[x])
    ...
    for x in range(num_runs):
        axs[1].plot(datasets[x][0]["age_ka"], datasets[x][0]["d18O_sw"], color=colours[x], linestyle='-', label="{} Run {}".format(datasets[x][1], (x + 1)))
        if fill:
            axs[1].fill_between(datasets[x][0]["age_ka"], datasets[x][0]["d18O_min1"], datasets[x][0]["d18O_plus1"], alpha=0.1, facecolor=colours[x])
