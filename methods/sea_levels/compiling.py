...
def sea_level_correlation(filter_period: float = 4.0, save_fig: bool = False):
    ...
    # Reduce the space between axes to 0
    fig.subplots_adjust(hspace=0)
    axs[0] = sea_level_plot(axs[0], age_min=2400, age_max=3400) # Plot up Sea Level Curve
    axs[1] = difference_plot(axs[1])
    axs[2] = filtered_difference_plot(axs[2])
    axs[3] = sea_level_correlation_plot(axs[3])
    axs[4] = sea_level_correlation_significance_plot(axs[4])
